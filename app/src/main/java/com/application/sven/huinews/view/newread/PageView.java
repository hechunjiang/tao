...
import com.application.sven.huinews.view.newread.anim.CoverPageAnim;
import com.application.sven.huinews.view.newread.anim.HorizonPageAnim;
import com.application.sven.huinews.view.newread.anim.NonePageAnim;
import com.application.sven.huinews.view.newread.anim.PageAnimation;
import com.application.sven.huinews.view.newread.anim.ScrollPageAnim;
import com.application.sven.huinews.view.newread.anim.SimulationPageAnim;
import com.application.sven.huinews.view.newread.anim.SlidePageAnim;
...
public class PageView extends View {
    ...
    private OnPayInfoLisenter mOnPayInfoLisenter;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mCenterRect == null) {
            mCenterRect = new RectF(mViewWidth * 1 / 5, 0, mViewWidth * 4 / 5, mViewHeight);
        }
        ...
        boolean isAuto = UserSpCache.getInstance(getContext()).getBoolean(UserSpCache.SAVE_AUTO_PAY);
        mPageLoader.setSelceted(isAuto);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if ((moveX == 0 && moveY == 0) || (Math.abs(moveX - downX) < scaledTouchSlop)) {
                    //章节支付信息点击事件，
                    if (mAdapter != null && mAdapter.isShowPay(mPageLoader.mCurChapterPos)) {
                        if (mPageLoader.getAutoSelectRectF() != null && mPageLoader.getAutoSelectRectF().contains(x, y)) {
                            if (mOnPayInfoLisenter != null) {
                                mPageLoader.setSelceted(!mPageLoader.isSelceted());
                                mOnPayInfoLisenter.onSelectedAuto(mPageLoader.isSelceted());
                                refreshPage();
                            }
                            downX = 0;
                            downY = 0;
                        ...
                        if (mPageLoader.getPayRectf() != null && mPageLoader.getPayRectf().contains(x, y)) {
                            if (mAdapter.isShowPay(currentSection) && mAdapter.payStatus(currentSection) == 2) {
                                if (mOnPayInfoLisenter != null) {
                                    mOnPayInfoLisenter.onClickPay(mAdapter.getChapterId(currentSection));
                                }

                            } else if (mAdapter.isShowPay(currentSection) && mAdapter.payStatus(currentSection) == 3) {
                                if (mOnPayInfoLisenter != null) {
                                    mOnPayInfoLisenter.onPayCurrChapter();
                                }
                            }
    ...
    /* public void setPayInfo(int chapterPrice, int banlce, int payStatus) {
         if (mPageLoader != null) {
             mPageLoader.setPayInfo(chapterPrice, banlce, payStatus);
         }
     }
 */
    ...
    public void setOnPayInfoLisenter(OnPayInfoLisenter mOnPayInfoLisenter) {
        this.mOnPayInfoLisenter = mOnPayInfoLisenter;
    }

    public interface OnPayInfoLisenter {
        void onSelectedAuto(boolean isSelected);

        void onClickPay(int chaperId);

        void onPayCurrChapter();
    }
    ...
    public void setBancle(int bancle) {
        if (mPageLoader != null) {
            mPageLoader.setBancle(bancle);
        }
    }

    public boolean isAutoPay() {
        if (mPageLoader != null) {
            return mPageLoader.isSelceted();
        }
        return false;
    }

    public void setAutoPay(boolean isAuto) {
        if (mPageLoader != null) {
            mPageLoader.setSelceted(isAuto);
        }
    }
    ...
    public void setStatus(int status) {
        if (mPageLoader != null) {
            mPageLoader.chapterError(status);
        }
    }
