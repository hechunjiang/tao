package com.application.sven.huinews.utils.update;
...
public class UpdateInfo {
                    ...
                    public void run() {
                        if (mVersion.getVersion_code() != null && mVersion.getVersion_code().length() > 0) {
                            if (AppInfo.getVersionCode(activity) < Integer.parseInt(mVersion.getVersion_code())) {
                                ...
                                if (!isHome) {
                                    ToastUtils.showLong(activity, "您使用的已经是最新版本了");
                                }
                            }
                        }
                    }
    ...
    private void installApk(File apk) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            uri = FileProvider.getUriForFile(activity.getApplicationContext(), "com.application.sven.huinews.fileProvider", apk);
        ...
        try {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (isClickUpdate) {
                        showNotify("下载中", (int) (fileSizeDownloaded * 1.0 / fileSize * 100));
                    }
                }
                outputStream.flush();
    ...
    public void updateApkEvent(FileLoadEvent event) {
        showNotify("下载中", (int) (event.getBytesLoaded() * 1.0 / event.getTotal() * 100));
    }
