...
public abstract class ReadView extends View {
    ...
    public ReadView(Context context, String bookId, OnReadStateChangeListener listener) {
        super(context);
        this.listener = listener;

        mScreenWidth = ScreensUitls.getScreenWidth(context);
        mScreenHeight = ScreensUitls.getScreenHeight(context);
    ...
    public synchronized void init(int theme) {
        if (!isPrepared) {
            try {
                pagefactory.setBgColor(ThemeManager.setReaderBgColor(theme));
                // TODO: 2018/7/6    自动跳转到上次阅读位置
                int pos[] = {1, 0, 0};
                int ret = pagefactory.openBook(pos[0], new int[]{1, 2});
                LogUtil.showLog("msg--章节加载状态：" + ret);
                if (ret == 0) {
                    LogUtil.showLog("msg--加载失败");
                    return;
                }
                ...
                if (actiondownX >= mScreenWidth / 3 && actiondownX <= mScreenWidth * 2 / 3
                    ...
                    if (actiondownX < mScreenWidth / 2) {// 从左翻
                        ...
                        if (status == BookStatus.NO_PRE_PAGE) {
                            ToastUtils.showLong(getContext(), "没有上一页啦");
                            return false;
                        ...
                        if (status == BookStatus.NO_NEXT_PAGE) {
                            ToastUtils.showLong(getContext(), "没有下一页啦");
                            return false;
                        } else if (status == BookStatus.LOAD_SUCCESS) {
                            abortAnimation();
                            pagefactory.onDraw(mNextPageCanvas);
    ...
    public void nextPage() {
        BookStatus status = pagefactory.nextPage();
        if (status == BookStatus.NO_NEXT_PAGE) {
            ToastUtils.showShort(getContext(), "没有下一页啦");
    ...
    public void prePage() {
        BookStatus status = pagefactory.prePage();
        if (status == BookStatus.NO_PRE_PAGE) {
            ToastUtils.showShort(getContext(), "没有上一页啦");
            return;
        } else if (status == BookStatus.LOAD_SUCCESS) {
            if (isPrepared) {
                pagefactory.onDraw(mCurrentPageCanvas);
    ...
    public synchronized void setFontSize(final int fontSizePx) {
        ...
        if (isPrepared) {
            pagefactory.onDraw(mCurrentPageCanvas);
            pagefactory.onDraw(mNextPageCanvas);
            ThemeManager.cacheFontSize(fontSizePx);
            postInvalidate();
        }
    }
